...
GameServer::GameServer(int port) : m_port(port) {
    ...
    for (int i = 0; i < GameLogic::MAX_PLAYERS; ++i) {
        ...
        m_pendingInputs[i].throttle = 0.0f;
        m_pendingInputs[i].steer = 0.0f;
    }
}
        ...
        while (std::getline(stream, line)) {
            ...
            if (msg.type == Protocol::MessageType::CONNECT) {
                ...
                if (globalPlayerId >= 0 && globalPlayerId < GameLogic::MAX_PLAYERS) {
                    ...
                    m_pendingInputs[globalPlayerId].throttle = msg.throttle;
                    m_pendingInputs[globalPlayerId].steer = msg.steer;
                }
            }
...
void GameServer::broadcastGameState() {
    ...
    // Count actual cars in the game, not network connections
    int connectedCount = state.cars.size();
...
std::string GameServer::serializeGameState(const Protocol::GameState& state, int connectedCount) {
    ...
    for (size_t i = 0; i < state.cars.size(); ++i) {
        const auto& car = state.cars[i];
        if (i > 0) oss << ",";
        oss << "{\"playerId\":" << car.playerId 
            << ",\"x\":" << car.position.x
            << ",\"y\":" << car.position.y
            << ",\"vx\":" << car.velocity.x
            << ",\"vy\":" << car.velocity.y
            ...
            << ",\"finished\":" << (car.finishedRace ? "true" : "false")
            << "}";
    }
    oss << "]";
    
    // Serialize finish order
    oss << ",\"finishOrder\":[";
    for (size_t i = 0; i < state.finishOrder.size(); ++i) {
        if (i > 0) oss << ",";
        oss << state.finishOrder[i];
    }
    oss << "]}\n";
...
Protocol::Message GameServer::parseMessage(const std::string& data) {
    ...
    if (typePos != npos) {
        ...
        // Extract throttle
        size_t throttlePos = data.find("\"throttle\":");
        if (throttlePos != npos) {
            throttlePos += 11; // Skip "throttle":
            msg.throttle = std::stof(data.substr(throttlePos));
        }
        
        // Extract steer
        size_t steerPos = data.find("\"steer\":");
        if (steerPos != npos) {
            steerPos += 8; // Skip "steer":
            msg.steer = std::stof(data.substr(steerPos));
        }
